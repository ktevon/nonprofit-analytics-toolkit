...
# A high alpha (12.0) means many one-timers and a few frequent fliers.
loyalty_weights = np.random.pareto(a=12.0, size=num_contacts) # Alpha changed from 2.0 to 12.0
loyalty_weights /= loyalty_weights.sum()

def generate_segmented_donations_fix1(target_n, df_contacts):
    ...
    for i in range(target_n):
        ...
        if i < num_contacts: # Acquisition logic
            ...
            # Generate a weighted seasonal date based on a valid year
            possible_years = [y for y in years if y >= acq_date[donor_id].year]
            close_year = np.random.choice(possible_years)
...
df_adhoc = generate_segmented_donations_fix1(num_adhoc_opps, df_contacts)
print(df_adhoc.info())
print(df_adhoc.head())
