...
# Pareto Loyalty Weights
# This determines WHO gets picked to give. 
# A high alpha (e.g., 3.0) means many one-timers and a few frequent fliers.
loyalty_weights = np.random.pareto(a=12.0, size=num_contacts)
loyalty_weights /= loyalty_weights.sum()

def generate_segmented_donations_fix2(target_n, df_contacts):
    ...
    for i in range(target_n):
        ...
        if i < num_contacts: # Acquisition logic
            ...
            donor_id = np.random.choice(contact_ids, p=loyalty_weights)
            ...
            # High probability for the first 3 years, and low probability for "long-term" retention
            weights = [10 if y <= first_year + 2 else 1 for y in years_remaining]
            weights = np.array(weights) / sum(weights) # Normalize

            # Pick the year using these weights
            close_year = np.random.choice(years_remaining, p=weights)

            # Proceed with the seasonal logic
            ...
            # Date Safety Catch
            # If close_date falls before acquisition, pick a random day in their remaining "tenure"
...
df_adhoc = generate_segmented_donations_fix2(num_adhoc_opps, df_contacts)
print(df_adhoc.info())
print(df_adhoc.head())
